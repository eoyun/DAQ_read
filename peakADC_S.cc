void peakADC_S (const int runnum, const int Mid, const int channel){
	...
	plot->SetStats(0);
	sprintf(pedname,"./ped/S%d_%d_%d.txt",runnum,Mid,channel);
	fp = fopen(pedname,"rt");
	fscanf(fp,"%lf",&ped);
	fclose(fp);
	...
	fclose(fp);
	sprintf(outname,"./out_peak/S%d_%d_%d.root",runnum,Mid,channel);
