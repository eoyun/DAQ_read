void ped_dist (const int runnum, const int Mid, const int channel){
	...
	TH1F *plot = new TH1F("plot", "Waveform", 4096, 0, 4096); 
	plot->SetStats(0);
	...
	for (evt=0;evt<nevt;evt++){
		...
		for (i = 1; i < 101; i++) {
			...
			plot->Fill(adc[i*32 + ch_to_plot]);
		}
		
	}
	//ped = ped/100.;
	fclose(fp);
	sprintf(outname,"./ped/ped_%d_%d_%d.root",runnum,Mid,channel);
	TFile * root_fp = new TFile(outname,"recreate");
	plot->Write();
	root_fp->Close();
