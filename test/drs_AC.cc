...
int main(int argc, char* argv[]){
	...
	TFile *f = new TFile(Form("./roots/drs_time_AC_%d.root",runnum),"recreate");
	...
	//for (int i = 0; i < 100; i++){
		...
		drs_stop = waveData->getDrsStop();
	        ACwaveform = wave_ADC_calib(waveform,drs_stop.at(drs_num),ADC_value);
		int max_wave = *std::max_element(ACwaveform.begin()+1,ACwaveform.begin()+1001);
		int min_wave = *std::min_element(ACwaveform.begin()+1,ACwaveform.begin()+1001);
		float cut_up = (max_wave - min_wave) * 0.7 + min_wave;
		float cut_down = (max_wave - min_wave) * 0.3 + min_wave;
		std::map<int, int> crossbin = FindCross(ACwaveform,cut_up,cut_down);
		...
		//	if (i == 1818)std::cout<<cut_up<<" | "<<cut_down<<" | "<<iter.first << " | " <<iter.second<<std::endl;
			if (tmp_flag + iter.second == 1){
				for (int j = tmp_bin; j<iter.first; j++){
					...
					delta_v.push_back(ACwaveform.at(j)-ACwaveform.at(j+1));
					
					// rise
					if (tmp_flag == 1){
						bin_array_rise.push_back(bin);
						int total_delta_v = ACwaveform.at(iter.first + 1) - ACwaveform.at(tmp_bin);
					      	int n_bins = iter.first + 1 - tmp_bin;
						delta_t_rise.push_back((double) (ACwaveform.at(j+1)-ACwaveform.at(j)) * 1000. / 5.12 * n_bins / total_delta_v);
					}
					// fall
					if (tmp_flag == 0){
						bin_array_fall.push_back(bin);
						int total_delta_v = ACwaveform.at(tmp_bin) - ACwaveform.at(iter.first + 1);
					      	int n_bins = iter.first + 1 - tmp_bin;
						delta_t_fall.push_back((double) (ACwaveform.at(j)-ACwaveform.at(j+1)) * 1000. / 5.12 * n_bins / total_delta_v);
					}
				}
			}
			tmp_bin = iter.first;
	...
	std::string filename_out = "/u/user/eoyun/DAQ/25.03.27/DAQ_read/time_calib/AC_MID_"+std::to_string(mid)+"_ch_"+std::to_string(ch)+".txt";
