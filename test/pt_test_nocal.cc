...
int main(int argc, char* argv[]){
	...
	fclose(fp);
	TFile* f_root = new TFile(Form("./roots/pt_test_nocalib_%d.root",runnum),"recreate");
	...
	for (int i = 0;i<nevt;i++){
		...
		for (int j = 0; j<(int)zerocross.size() - 2; j=j+2 ){
			...
			double time_cor =  - abs(pedcor_wave.at(zerocross.at(j))/(pedcor_wave.at(zerocross.at(j)) - pedcor_wave.at(zerocross.at(j)+1) ))*1000. / 5.12 + abs(pedcor_wave.at(zerocross.at(j+1))/(pedcor_wave.at(zerocross.at(j + 2)) - pedcor_wave.at(zerocross.at(j+2)+1) ))*1000. / 5.12;
			h->Fill(start_bin,(bin_diff * 1000. / 5.12 + time_cor)/1000.);
