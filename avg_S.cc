void avg_S (const int runnum, const int Mid, const int channel){
	...
	for (evt=0;evt<nevt;evt++){
		...
		for (i = 0; i < 1023; i++) {
			plot->Fill(i, (double)adc[i * 32 + ch_to_plot]/nevt);
		}
		
	}
	//plot->Draw("hist");
	fclose(fp);
	TFile *fp_root = new TFile(Form("./avg/S%d_%d.root",runnum,channel),"recreate");
	plot->Write();
	fp_root->Close();
}
